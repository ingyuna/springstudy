...
@WebListener	// @WebListener는 지우면 안됨
public class BoardListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce)  { 	// 실행 시작할 때
    	...
    	try {   		
    		JobDetail job = JobBuilder.newJob(BoardJob.class)
    				.withIdentity("myBoardJob", "myGroup")		// withIdentity는 식별자 이름이기때문에 아무 이름이나 상관없다.
    				.build();		
    ...
    public void contextDestroyed(ServletContextEvent sce)  { 	// 실행 끝났을 때
