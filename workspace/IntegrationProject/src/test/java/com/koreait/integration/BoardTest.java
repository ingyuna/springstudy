...
public class BoardTest {
	...
	public void test() {
		...
		try {
			...
			String apiURL = "http://localhost:9090/integration/selectQuery.do?column=TITLE&query=" + query;		// 공지를 따로 빼는 이유 = 위에서 인코딩하기 위해서	
																	
			URL url = new URL(apiURL);
			HttpURLConnection con = (HttpURLConnection)url.openConnection();			// connection을 만든이유 : 데이터를 다 읽어올 수 있게
			BufferedReader br = new BufferedReader( new InputStreamReader( con.getInputStream() ) )  ;
														// == byte기반의 스트림을 text기반의 스트림으로 바꿔준다.
			String resultMap = "";
			String line = null;
