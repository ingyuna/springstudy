...
public class BoardController {
	...
	private UpdateBoardCommand updateBoardCommnad;
	...
	public void setCommand(SelectBoardListCommand selectBoardListCommand,
						   ...
						   UpdateBoardCommand updateBoardCommnad,
		...
		this.updateBoardCommnad = updateBoardCommnad;
	...
	public String updateBoard(HttpServletRequest request,		//update.jsp에서 전달한 파라미터
		...
		updateBoardCommnad.execute(model);
		return "redirect:selectBoardByNo.do?no=" + request.getParameter("no");	// selectBoardByNo.do 맵핑으로 리다이렉트
	}
	
											// 삽입, 수정, 삭제는 redirect를 하자!
	...
	public String deleteBoard(@RequestParam("no") long no,		// 삭제할 게시글 no
		...
		return selectBoardList(model);				// 메소드 호출 
								// (-> redirect는 어디를 거쳐서 그 다른 맵핑의 메소드를 실행한다는건데 
								// 그 맵핑에 해당하기 때문에 요 메소드를 불러서 해줘도 된다)	
	}
	...
	@GetMapping(value="insertBoard.do")		// insert.jsp 파일에 insertBoard.do는 메소드가 없으니까 @GetMapping
