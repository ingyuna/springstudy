...
public class PagingUtils {
	...
	public static PageDTO getPage(int totalRecord, int page) {	// totalRecord와 page는 스스로 알 수 없다. 매개변수로 전달을 받아야 알 수 있음.
	...
	// << 1 2 3 4 5 >> 반환
	public static String getPaging(String path, int page) {		// path에는 컬럼(?x=)과 쿼리(&q=)가 포함되어잇다.
		
		StringBuilder sb = new StringBuilder();
		
		if (beginPage <= pagePerBlock) {	// 1블록이라서 이전이 없음 (=> 클릭이 안 됨)
			sb.append("◀&nbsp;");
		} else {
			if (path.indexOf("?") > 0) {
				sb.append("<a href=\"" + path + "?page=" + (beginPage - 1) + "\">◀</a>&nbsp;");				
							// = <a href="searchAll.do?page=1">
			} else {
				sb.append("<a href=\"" + path + "?page=" + (beginPage - 1) + "\">◀</a>&nbsp;");				
			}
		}
