	...
	<script>
		...
		function fn_selectMemberList() {
			...
			$.ajax({
				...
				success: function(resultMap) {
					...
					if (resultMap.exists) {
						// resultMap.list 출력
						$.each(resultMap.list, function(i, member){		// i = index
							$('<tr>')
							// .append('<td>' + member.id + '</td>')	// .append('<td>').text(member.id) => 같은 코드 (이걸로 해도 됨)
							.append( $('<td>').text(member.id) )		// text대신 html로 넣어줘도 된다(태그가 사용된다면)
							.append( $('<td>').text(member.name) )
							.append( $('<td>').text(member.address) )
							.append( $('<td>').text(member.gender) )	// 닫아주는 태그(</td>) 없어도 됨.
						...
						/*
						$('#member_list')
						.append( $('<tr>').append('<td colspan="5">등록된 회원이 없습니다.</td>') );
						*/    // 밑에가 나음 
						$('<tr>')
		...
		function fn_paging() {
			$('body').on('click', '.previous_block', function(){
				page = $(this).attr('data-page');		// $(this).data('page') 이렇게 해도 된다.
				fn_selectMemberList();
			});
		...
		function fn_selectMemberByNo() {
			$('body').on('click', '#view_btn', function(){	// 위에 view_btn이 동적으로 만들어졌기 때문에 이렇게 해준다.			
				$('input:text[name="id"]').attr('readonly', true);
				$.ajax({
					url: 'member/' + $(this).prev().val(),		// -> 경로(주소창)에다가 파라미터를 가지고 간다.
					type: 'get',
					dataType: 'json',
					success: function(resultMap) {
						if (resultMap.exists) {
		...
		function fn_insertMember() {
			$('#insert_btn').click(function(){
				...
				$.ajax({
					...
					data: JSON.stringify(member),	// JSON 전달 (보내는 데이터)
					dataType: 'json',				// 받는 데이터
					success: function(resultMap) {		// 받아올 곳 
					...
					error: function(xhr, testStatus, errorThrown) {
						...
						*/
						...
						case 1001:
							alert(xhr.responseText);
							break;			// break는 있어도 되고, 없어도 된다. 상관없음.
						}
					}
				});
			});
		...
		function fn_init() {
			$('#init_btn').click(function(){
				...
				$('input:text[name="name"]').val('');
				$('input:text[name="address"]').val('');
				$('input:radio[name="gender"]').prop('checked', false);		// 선택만 없애준다. val()를 없애면 남자/여자가 다 없어져서 안 됨.
			});
		}
