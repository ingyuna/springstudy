...
public class BoardController {
	...
	private SqlSession sqlSession;		// SqlSessionTemplate 클래스는 SqlSession인터페이스를 구현한 클래스 (-> 그래서 두가지 다 가능하다)		
	...
	@GetMapping(value="/")		// contextPath로 어디를 갈건지 먼저 설정
	public String index() {
		return "index";
	}	
	...
	@GetMapping(value="selectBoardByNo.do")		// a 링크를 통한 mapping이니까 @GetMapping으로 해준다.
	...
	public String updateBoardPage(Board board,
			                      Model model) {	// @ModelAttribute Board board -> 이걸 쓰면, 
													// 스스로 모델에 담아주는 model.addAttribute까지 해준다.
