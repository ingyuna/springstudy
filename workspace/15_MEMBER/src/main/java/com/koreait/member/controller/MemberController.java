...
public class MemberController {
	...
	@ResponseBody
	public Map<String, Integer> idChceck(HttpServletRequest request, 
					      Model model) {
	...
	public String leave(HttpSession session,
		...
		return "redirect:/";	// -> return index();	 이렇게 해도 됨(=> 위에 index() 코드 호출)
	}
	...
	@PostMapping(value="presentPwCheck.do",
				 produces="application/json; charset=utf-8")
	@ResponseBody
	public Map<String, Boolean> presentPwCheck(@RequestBody Member member,	// 파라미터 없이 json으로 전달받아 올 때는 @RequestBody로.
												HttpSession session,
												Model model) {
		model.addAttribute("session", session);
