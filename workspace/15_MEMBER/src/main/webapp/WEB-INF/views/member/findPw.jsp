		...
		function fn_verify(authCode) {
			$('#verify_btn').click(function(){
				if (authCode == $('#user_key').val()) {
					...
					$('#f').attr('action', 'changePwPage.do');	// changePwPage.do'는 단순 페이지 이동이 아니라 데이터(email)를 가지고 가야한다.-->
					$('#f').submit();
				} else {
					alert('인증이 실패했습니다.');
					history.back();
...
<body>
	...
	<form id="f">
		가입 당시 이메일을 입력하세요<br>
		<input type="text" name="email" id="email">		<!-- email이 unique이기 때문에, 식별자가 되어 조건으로 사용될 수 있다. -->
